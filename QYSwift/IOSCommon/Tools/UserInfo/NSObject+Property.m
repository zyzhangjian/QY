...
+ (instancetype)zjobjectWithDictionary:(NSDictionary *)dictionary
{
    ...
    for (unsigned int i = 0; i < count; i++)
    {
        ...
        if ([value isKindOfClass:[NSDictionary class]])
        {
            ...
            if (modelClass)
            {
                value = [modelClass zjobjectWithDictionary:value];
            }
        }
        ...
        if ([value isKindOfClass:[NSArray class]])
        {
            if ([self respondsToSelector:@selector(objectClassInArray)])
            {
                ...
                {
                    id model = [classModel zjobjectWithDictionary:dict];
